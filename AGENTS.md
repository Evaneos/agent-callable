...
Semver unique `v<major>.<minor>.<patch>` pour binaire et plugin.
Source de vérité : `plugins/agent-callable/.claude-plugin/plugin.json` → champ `version`.
Tag git : `make tag` lit plugin.json et crée le tag annoté.
GoReleaser tourne en CI (GitHub Actions) sur push de tag — ne pas le lancer localement.
`make build-all` et `make package` servent au dev local uniquement.
